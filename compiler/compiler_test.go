...
func TestStringExpressions(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             `"def"`,
			expectedConstants: []interface{}{"def"},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpPop),
			},
...
`,
		...
		{
			input: `
def manyArg = fn(a, b, c) { }; manyArg(24, 25, 26);
`,
			expectedConstants: []interface{}{
				[]code.Instructions{
					code.Make(code.OpReturn),
...
`,
		...
		{
			input: `
def manyArg = fn(a, b, c) { a; b; c }; manyArg(24, 25, 26);
`,
			expectedConstants: []interface{}{
				[]code.Instructions{
					code.Make(code.OpGetLocal, 0),
...
func TestDefStatementScopes(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `
def num = 55;
...
func TestRecursiveFunctions(t *testing.T) {
	tests := []compilerTestCase{
		{
			input: `
def countDown = fn(x) { countDown(x - 1); };
countDown(1);
`,
			expectedConstants: []interface{}{
				1,
